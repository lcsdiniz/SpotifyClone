...
const ResultsTabs = (props) => {
  ...
  useEffect(() => {
    setArtists(props.artists.map((artist, key) =>
      <Columns.Column desktop={{ size: 3 }} mobile={{ size: 6 }} key={key}>
        <Artist artist_name={artist.name} photo_url={artist.photo_url} id={artist.id}/>
      </Columns.Column>
    ));
  }, [props.artists]);
  ...
  return (
    <Fragment>
      <Tabs style={{ display: props.albums.length || props.artists.length || props.songs.length ? "" : "none" }} align='centered' size='medium'>
